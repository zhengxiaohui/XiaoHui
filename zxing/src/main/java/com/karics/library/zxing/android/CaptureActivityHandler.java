...
import com.karics.library.zxing.my.QRCodeConst;
...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		switch (message.what) {
			case QRCodeConst.RESTART_PREVIEW:
				// 重新预览
				restartPreviewAndDecode();
				break;
			case QRCodeConst.DECODE_SUCCEEDED:
				// 解码成功
				state = State.SUCCESS;
				...
				break;
			case QRCodeConst.DECODE_FAILED:
				...
				cameraManager.requestPreviewFrame(decodeThread.getHandler(),
						QRCodeConst.DECODE);
				break;
			case QRCodeConst.RETURN_SCAN_RESULT:
				//扫描结果，返回CaptureActivity处理
				activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
				activity.finish();
				break;
			case QRCodeConst.LAUNCH_PRODUCT_QUERY:
	...
	public void quitSynchronously() {
		...
		Message quit = Message.obtain(decodeThread.getHandler(), QRCodeConst.QUIT);
		quit.sendToTarget();
		try {
		...
		//确保不会发送任何队列消息
		removeMessages(QRCodeConst.DECODE_SUCCEEDED);
		removeMessages(QRCodeConst.DECODE_FAILED);
	}
