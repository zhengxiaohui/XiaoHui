...
import com.zbase.R;
import com.zbase.interfaces.FlowIndicator;
 ...
 * </ul>
 */
public class RectSurroundFlowIndicator extends View implements FlowIndicator,
        AnimationListener {
    private static final int STYLE_STROKE = 0;
    private static final int STYLE_FILL = 1;

    private float radius = 4;
    private int surround=5;
    ...
    private final Paint mPaintActiveSurround = new Paint(Paint.ANTI_ALIAS_FLAG);
     ...
     * @param context
     */
    public RectSurroundFlowIndicator(Context context) {
        super(context);
     ...
     * @param attrs
     */
    public RectSurroundFlowIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initColors(int activeColor, int inactiveColor, int activeType,
        ...
        mPaintActiveSurround.setStyle(Style.STROKE);
        mPaintActive.setColor(activeColor);
        mPaintActiveSurround.setColor(activeColor);
        mPaintActiveSurround.setStrokeWidth(2);
    }
...
//                    getPaddingTop() + radius, radius, mPaintInactive);
            float left=leftPadding+ (iLoop * circleSeparation) + centeringOffset+surround;
            float top=getPaddingTop()+surround;
            canvas.drawRect(left,top,left+radius*2-surround*2,top+radius*2-surround*2,mPaintInactive);
        }
        float cx = 0;
        if (flowWidth != 0) {
...
//                + radius, radius + activeRadius, mPaintActive);
        float left=leftPadding + cx + centeringOffset+surround;
        float top=getPaddingTop()+surround;
        canvas.drawRect(left,top,left+radius*2-surround*2,top+radius*2-surround*2,mPaintActive);

        float surroundLeft=leftPadding + cx + centeringOffset;
        float surroundTop=getPaddingTop();
        canvas.drawRect(surroundLeft,surroundTop,left+radius*2-surround,top+radius*2-surround,mPaintActiveSurround);
    }
