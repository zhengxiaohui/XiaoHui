...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    public void setIsAutoFlow(boolean isFlow) {
        this.isAutoFlow = isFlow;
    }
    ...
    public boolean isAutoFlowTimer() {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (getChildCount() == 0)//如果没有子控件就不用拦截
            return false;
...
//                getParent().requestDisallowInterceptTouchEvent(true);
                ...
                boolean xMoved = xDiff > mTouchSlop;//滑动的距离是否足够判定为滑动
                ...
                if (mTouchState == TOUCH_STATE_SCROLLING) {//如果是在滑动
                    ...
                    return true;//如果是滑动就拦截
                } else {
                    return false;//不是滑动就不拦截
                }
                //break;
...
//                getParent().requestDisallowInterceptTouchEvent(true);
    ...
    @Override
    public boolean onTouchEvent(MotionEvent ev) {//直接消费，不传递给父控件

        if (getChildCount() == 0)//如果没有子控件就直接消费
            return true;
